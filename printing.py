...
def print_summary(all_threads_stats, bytes_cov, time_elapsed, active_threads_count, args, update, mutators, gui_state=None):
    ...
    # Update stats in shared gui_state
    if gui_state:
        gui_state.set_execs_per_sec(stats_total.stats['exec_per_sec'])
        gui_state.set_total_execs(int(stats_total.stats['executions']))
        gui_state.set_total_time(time_elapsed)
        if isinstance(time_since_last_path, timedelta):
            time_since_last_path = float(time_since_last_path.seconds + time_since_last_path.microseconds / 1000000)
        gui_state.set_since_last_path(time_since_last_path)
        if isinstance(time_since_last_crash, timedelta):
            time_since_last_crash = float(time_since_last_crash.seconds + time_since_last_crash.microseconds / 1000000)
        gui_state.set_since_last_crash(time_since_last_crash)
        gui_state.set_files_in_queue(int(stats_total.stats['files_in_queue']))
        gui_state.set_new_paths(int(stats_total.stats['new_paths']))
        gui_state.set_crashes(int(stats_total.stats['crashes']))
        gui_state.set_unique_crashes(int(stats_total.stats['unique_crashes']))

    if gui_state is None or gui_state.check_alive() is False:
        print(bcolors.BOLD)
...
def print_bitmaps(bitmap_original, bitmap, output_file_path):
    ...
    if output_file_path:
        with open(output_file_path, 'rb') as f:
            data = f.read()
        content = data.decode('utf-8', errors="backslashreplace").split('\n')
        fd.write("--------------------------------------------\n")
